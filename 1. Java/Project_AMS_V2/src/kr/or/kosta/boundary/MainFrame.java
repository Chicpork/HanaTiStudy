...
public class MainFrame extends Panel {
	...
	public void setManager(AccountDao manager) {
		this.accountDao = manager;
	}
	...
	public void getAccount() {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "파일 입출력 프로그램 작동시 에러 발생", "에러!", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public void removeAccount() {
		...
		try {
			isEmptyAccountNum();
			if (!accountDao.removeAccount(accountNum)) {
	...
	public void searchAccount() {
		...
		try {
			isEmptyAccountOwner();
			List<Account> lists = accountDao.searchAccount(accountOwner);
			printList(lists);
		} catch (AccountException error) {
			accountListTA.setText(error.toString());
		} catch (IOException e) {
	...
	public void openAccount() {
		try {
			...
			if (selectedAccountType.equals("입출금계좌")) {
				...
				Account account = new Account(accountNumTF.getText().trim(), accountOwnerTF.getText().trim(), passwd,
						depositMoney);
				...
				Account account = new MinusAccount(accountNumTF.getText().trim(), accountOwnerTF.getText().trim(),
						passwd, depositMoney-borrowMoney, borrowMoney);
				accountDao.openAccount(account);
				accountListTA.setText("계좌 등록이 완료되었습니다.");
			}
		} catch (AccountException error) {
			...
			accountListTA.setText("비밀 번호는 " + Integer.MAX_VALUE + " 까지 가능합니다.\n입금 금액은 최대 " + Long.MAX_VALUE + " 까지만 입금가능합니다.\n대출 금액은 최대 " + Long.MAX_VALUE + " 까지만 입금가능합니다.");
	...
	public void listAll() {
		try {
			List<Account> lists = accountDao.listAccount();
			printList(lists);
		} catch (AccountException error) {
			accountListTA.setText(error.toString());
		} catch (IOException e) {
	...
	public void finish() {
		try {
			accountDao.closeAccountDao();
		} catch (IOException e) {
			JOptionPane.showMessageDialog(null, "파일 입출력 프로그램 종료시 에러 발생", "에러!", JOptionPane.ERROR_MESSAGE);
		}
